				...
				if (column.getType().getSuperclass() == ActiveRecordBase.class)
				{
					values.put(CamelNotationHelper.toSQLName(column.getName()),
							column.get(this) != null ? 
									String.valueOf(((ActiveRecordBase) column.get(this))._id) : "0");
			    }
				else if (column.getType().getName().equals("[B"))
				{
					values.put(CamelNotationHelper.toSQLName(column.getName()), (byte[])column.get(this));
				}
				else
				{
					values.put(CamelNotationHelper.toSQLName(column.getName()), String.valueOf(column.get(this)));					
				}
			} catch (IllegalAccessException e) {
